function GameModel() {
  ...
  this.gameStatus = function (start) {
    ...
    if (status) {
      ...
      mess.y = -car.h;
      viewM.overGame();
      setTimeout(function (){viewM.renderMess(mess.x, mess.y);},1000);
    }
  };
  ...
  this.deleteUser = function () {
    database.ref("users/" + user.name).remove()
      .then(function () {
  ...
  this.reloadGame = function () { // перезагрузка гонки
    //перзагрузка игры
    start = false;
    // console.log('reload')
    viewM.overGame();
  ...
  this.closeGame = function () { //перход на первую страницу
    //   console.log('close')
    viewM.clearRender();
    viewM.renderStartPage();
  };

  this.loadStartMessage = function () { //рендер игры
    //   console.log('click')
    viewM.clearRender();
    viewM.render();
  };
