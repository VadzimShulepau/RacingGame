function GameView() {
  ...
  this.renderTouchArrow = function (){ //кнопки для управления на сенсорном экране
  ...
  this.carController = function (x, y, deg) {
    car.cor = deg;
    if (x > 0 + carWrapper.offsetWidth / 5 && x < track.offsetWidth - carWrapper.offsetWidth - carWrapper.offsetWidth / 5) {
      car.x = x;
    }
    if (y > track.offsetTop && y < track.offsetHeight - carWrapper.offsetHeight) {
      car.y = y;
    }
  ...
  this.renderRoad = function (pos) { // обновление трассы
    //отрисовка дороги
  ...
  this.clearRender = function () {// очищаем контейнер
    wrapper.innerHTML = "";
    //   gameWrapper.classList.toggle('hide');
  };

  this.renderStartPage = function () { // верска стартовой страницы
    if (!wrapper) {
      wrapper = document.createElement("div");
      wrapper.classList.add("wrapper");
      bodyV.append(wrapper);
  ...
  this.deleteUser = function (){ // сообщение при удалении данных пользователя
    deleteUserMessage.textContent = 'Пользователь удален';
  };

  this.gameViewTouch = function (){ //замена картинки для сенсорных дисплеев
    descriptionArrow.textContent = '';
    arrowIMG.src = './png/touch.png'; 
  };

  this.soundClick = function (check){ //звук при клике на объекты
    bip.play();
  };
